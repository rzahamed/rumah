...
| Posts & categories | Draft/published with a required publish date for published posts; `published()` scope hides future-dated posts; deleting a category never deletes posts. Optional featured image on the env-driven `BLOG_FEATURED_DISK`/`BLOG_FEATURED_DIR` with a commit-safe lifecycle and rollback-safe upload cleanup (page-scoped transactions + an exact-path ownership ledger for freshly stored files). |
| Team members | Sort order, visibility, photo and licence-image uploads on the configured media disk with commit-safe file lifecycle: replacement and deletion remove files only after the DB change commits, guarded by a committed-state reference check across both media columns. Detail fields for rich profiles: public email, locale-keyed highlights/credentials/expertise lists with locale-resolving read helpers that drop empty items. |
| Forms & submissions | Admin-defined forms (7 field types incl. `select` and `checkbox`, per-locale labels, unique machine names, max 20 fields). Select options are bounded, machine-valued, and validated server-side against a sanitized allowlist (`Rule::in`); a required checkbox enforces consent with the `accepted` rule; checkbox payloads store explicit booleans. Public submission endpoints (default + localized) validate against the form's own definition, store only declared fields, collect no requester metadata, and are rate limited. Submissions are protected admin data: list/view/single-delete only — no create/edit routes, no bulk actions; forms with submissions cannot be deleted (`restrictOnDelete`). |
| FAQs | Locale-keyed question/answer, sort order, visibility; `visible()` scope returns public items in deterministic order (sort order, then id). Managed by admins and editors via the standard content permission set (`faqs.*`). |
| Site settings | Super-admin-only single-row settings (database-enforced singleton). Four trusted raw-HTML custom-code snippets rendered at exactly four public-layout positions — never on the admin host, never logged, never serialized — plus an allowlist-validated HTTPS booking URL (`CAL_ALLOWED_HOSTS`). Authorization comes solely from the active-super-admin `Gate::before` override; no role can be granted access. |
